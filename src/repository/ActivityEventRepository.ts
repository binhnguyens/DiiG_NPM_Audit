...
export class ActivityEventRepository {
  ...
  public static async _insert(participant_id: string, objects: ActivityEvent[]): Promise<{}> {
	...
	static_data: (process.env.Activity_Event_Encryption === "on" ?
                     CryptoJS.AES.encrypt(JSON.stringify(x.static_data),`${process.env.ENC_KEY}`).toString():x.static_data)??{},
        temporal_slices: (process.env.Activity_Event_Encryption === "on" ?
                     CryptoJS.AES.encrypt(JSON.stringify(x.temporal_slices),`${process.env.ENC_KEY}`).toString():x.temporal_slices)
        ?? [],
      })),
    })
